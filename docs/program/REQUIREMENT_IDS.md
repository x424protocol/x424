...
| ------------- | -------------------------------------------------------------------------- | ------------ |
| X424-HTTP-001 | Unsatisfied dependency returns 424 with HUMAN-REQUIRED                     | covered      |
| X424-HTTP-002 | Cache-Control no-store, private on challenges/results                      | covered      |
| X424-HTTP-003 | Vary includes HUMAN-PROOF on 424                                           | covered      |
| X424-HTTP-004 | Content-Type application/problem+json on 424 body                          | covered      |
| X424-HTTP-005 | Header size capped; invalid base64url/JSON rejected                        | covered      |
| X424-HTTP-006 | Provider-native proofs MUST NOT be placed in HTTP headers                  | covered      |
| X424-HTTP-007 | Identify x424 by HUMAN-REQUIRED + version, not status alone                | needs_vector |
| X424-HTTP-008 | CORS: explicit origins; expose HUMAN-* headers; credentials documented     | needs_vector |
| X424-HTTP-009 | Redirects MUST NOT forward challenges/proofs to unintended origin/audience | needs_vector |
| X424-HTTP-010 | Conservative inline requirement envelope; fail closed above limit          | needs_vector |
| X424-HTTP-011 | Alternate body/reference transport if inline unreliable (versioned)        | needs_vector |
...
| X424-REQ-003 | requestDigest covers method, uri, body digest                                  | covered                        |
| X424-REQ-004 | audience identifies accepting resource server                                  | covered                        |
| X424-REQ-005 | expiresAt later than createdAt; ref profile 30–900s                            | covered                        |
...
| X424-REQ-009 | Absent body → bodyDigest null; present JSON → sha256(canonical-json)           | covered                        |
| X424-REQ-010 | Non-JSON / empty / binary / multipart digest profile (see ADR)                 | needs_vector                   |
| X424-REQ-011 | Streams and non-cloneable bodies: client fail-closed or explicit digest mode   | needs_vector                   |
| X424-REQ-012 | Idempotency-Key SHOULD be required for mutations                               | needs_vector                   |
...
| X424-MTH-002  | Unknown/disabled/stale/wrong-scope/mode fail closed       | covered                    |
...
| X424-BIND-004 | Copying binding from client JSON is not verification      | needs_vector               |
...
| X424-RES-003 | Never accept key presented alongside its token                | needs_vector |
...
| X424-REPLAY-002 | Result consumption does not replace app idempotency                   | needs_vector |
| X424-PRIV-001   | Raw proofs/nullifiers never in logs/traces/analytics/queues/results   | needs_vector |
| X424-PRIV-002   | Pairwise IDs differ across audiences and methods                      | covered      |
| X424-PRIV-003   | Error bodies MUST NOT echo proofs/nullifiers/secrets/diagnostics      | needs_vector |
...
| X424-VERNEG-003 | Algorithm allowlist; reject confusion           | needs_vector            |
